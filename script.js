/*
    This file is a part of Waleed's Portfolio
    Copyright (C) 2024 Waleed Ahmed
...
console.log("Welcome to waleed's portfolio !")

title = "Hi there, I'm Waleed Ahmed. Welcome to my portfolio !           "
position = 0
function scrolltitle () {
  document.title =
    title.substring(position, title.length) + title.substring(0, position)
  position++
...
for (var ctmi = 0; ctmi < cmlabel.length; ctmi++) {
  cmlabel[ctmi].innerHTML = cmlabel[ctmi].innerText
    .split('')
    .map(
      (cmlet, cml) =>
        `<span class="inputword" style="transition-delay: ${
          cml * 30
        }ms;filter: hue-rotate(${cml * 25}deg)";>${cmlet}</span>`
    )
    .join('')
}
